...
class CTABGANSynthesizer:
    ...
    def fit(self, train_data=pd.DataFrame, categorical=[], mixed={}, type={}):
        ...
        # if cache != "":
        #     cache_path = 'pickles/'
        #     all_pickles = glob.glob(cache_path + f"*{cache}.pickle")
        #     discriminators = [x for x in all_pickles if "discriminator" in x]
        #     generators = [x for x in all_pickles if "generator" in x]
        #     # use the last generator and discriminator as highest cached epoch
        #     self.discriminator.load_state_dict(torch.load(discriminators[-1]))
        #     self.discriminator.eval()
        #     self.generator.load_state_dict(torch.load(generators[-1]))
        #     self.generator.eval()
        ...
        for i in tqdm(range(self.epochs)):
            for _ in range(steps_per_epoch):
                ...
                if problem_type:
                    ...
                    optimizerC.step()
                    # print(f"Epoch: {i}")
                    # print(f"Loss cc: {loss_cc.item()}")
                    # print(f"Loss cg: {loss_cg.item()}")
                    # print(f"Loss info: {loss_info.item()}")
                    # print(f"Loss d: {loss_d.item()}")
